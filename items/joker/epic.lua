SMODS.Joker{
    ...
    loc_txt = {
        name = 'Warlock',
        text = {
            'This joker gains {X:purple,C:white}X#2#{} Score',
			'per {C:attention}hand {}played.',
			'Loses {X:purple,C:white}X#3#{} Score per {C:attention}card {}discarded',
	...
	loc_vars = function(self, info_queue, card)
		return {
			vars = {card.ability.extra.score, card.ability.extra.score_mod, -card.ability.extra.score_loss}
		}
	end,
	calculate = function(self, card, context)
		if context.before or context.forcetrigger then
			SMODS.scale_card(card, {ref_table = card.ability.extra, ref_value = "score", scalar_value = "score_mod"})
		end
		if context.discard then
			SMODS.scale_card(card, {ref_table = card.ability.extra, ref_value = "score", scalar_value = "score_loss"})
		end
